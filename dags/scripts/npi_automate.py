    ...
    try:
        ...
        with log_step(logger, f"download zip from {url}"):
            ...
            if response.status_code == 404:
                raise FileNotFoundError(f"CMS NPPES dump not yet published at {url}")
...
def process_extracted_npi_data_v2():
    ...
    if not zip_file_path:
        raise FileNotFoundError(
            "No zip file found in working directory - did the download step run?"
        )

    log_file_info(logger, zip_file_path, label="source zip")

    #DuckDb Connection through hook
